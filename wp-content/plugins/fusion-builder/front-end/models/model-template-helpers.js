...
_.mixin( {
			...
			_.each( socialNetworks, function( value ) {
				...
				_.each( value, function( link, network ) {
					...
					if ( 'custom' === network && link ) {
						...
						let socialMediaIcons = params.social_media_icons;
						if ( 'undefined' !== typeof socialMediaIcons && 'undefined' !== typeof socialMediaIcons.icon && 'undefined' !== typeof socialMediaIcons.icon_mark ) {
							const networkSearchName = 'mail' === network ? 'email' : network,
								key                 = Number( Object.keys( socialMediaIcons.icon ).find( ( k ) => socialMediaIcons.icon[ k ] === networkSearchName ) );

							if ( key && 'undefined' !== typeof( socialMediaIcons.icon_mark[ key ] ) ) {
								iconOptions.icon_mark = socialMediaIcons.icon_mark[ key ].replace( 'fusion-prefix-', '' )
							}
						}
	...
	fusionGetBackgroundSliderElement: function( self, type ) {
		...
		if ( 'yes' === values.background_slider_random_order ) {
